...
do_install() {
	...
	ln -sf ${systemd_unitdir}/system/init_sys_mss_rproc.service ${D}${systemd_unitdir}/system/local-fs.target.wants/init_sys_mss_rproc.service
        ...
        if [[ ${@bb.utils.contains_any('DISTRO_NAME', 'mdm auto', 'true', 'false', d)} || \
           ...
           sed -i '/Requires=firmware-mount.service/a Before=sockets.target' ${D}${systemd_unitdir}/system/init_sys_mss.service
            ...
            if [ "${MACHINE}" == "sdxlemur" ]; then
                sed -i '/RemainAfterExit=yes/a ExecStartPost=+sleep 15' ${D}${systemd_unitdir}/system/init_sys_mss.service
            else
